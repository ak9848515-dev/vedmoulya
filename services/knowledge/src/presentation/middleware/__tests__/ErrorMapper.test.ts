// ─────────────────────────────────────────────────────────────────────────────
// VedMoulya — Knowledge ErrorMapper unit tests
...
describe('mapErrorToResponse', () => {
  ...
  it('includes AppError details when present', () => {
    const c = makeCtx();
    const err = new NotFoundError('Graph', 'graph_1');
    const response = mapErrorToResponse(err, c);
    expect((response.body as { error: { message: string } }).error.message).toContain('graph_1');
  ...
  it('maps a non-Error unknown value to INTERNAL_ERROR with 500', () => {
    const c = makeCtx();
