...
describe('MemoryCache', () => {
  ...
  beforeEach(() => {
    vi.useFakeTimers();
    cache = new MemoryCache(5000); // 5 second default TTL
  });
  ...
  it('invalidateByPrefix removes matching entries', () => {
    cache.set('memory:1', 'first');
    cache.set('memory:2', 'second');
    cache.set('config:1', 'dark');

    cache.invalidateByPrefix('memory:');
    expect(cache.get('memory:1')).toBeUndefined();
    expect(cache.get('memory:2')).toBeUndefined();
