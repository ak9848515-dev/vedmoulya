// ──────────────────────────────────────────────────────────────────
// VedMoulya — Memory Error Mapping Middleware
...
export function mapErrorToResponse(error: unknown, c: Context): Response {
  if (error instanceof AppError) {
    logger.warn('Memory application error', {
      code: error.code,
  ...
  if (error instanceof Error) {
    logger.error('Memory unexpected error', {
      message: error.message,
      stack: error.stack,
    });
...
export const errorMiddleware = async (c: Context, next: () => Promise<void>): Promise<void> => {
  try {
    ...
    // response is already sent via c.json in mapErrorToResponse
    void response;
  }
};
